...
class DismantleSubscriptionController extends Controller
{
    /**
     * Display DISMANTLE subscriptions with map view
     */
    public function index(Request $request): Response
    {
        // Only get basic statistics and initial center coordinates for map
        $stats = $this->getDismantleSubscriptionStats();
        ...
        return Inertia::render('Admin/DismantleSubscription/Index', [
            'customers' => $customers,
    ...
    private function getDismantleSubscriptionStats(): array
    {
        $totalDismantle = Subscription::where('subscription_status', 'DISMANTLE')->count();

        $withCoordinates = Subscription::where('subscription_status', 'DISMANTLE')
            ->where('subscription_maps', '!=', '-')
            ->whereNotNull('subscription_maps')
            ->where('subscription_maps', '!=', '')
            ->count();

        $withoutCoordinates = $totalDismantle - $withCoordinates;

        return [
            'total_dismantled' => $totalDismantle,
    ...
    private function getInitialMapData(Request $request): array
    {
        $query = Subscription::select([
            ...
            ->where('subscription_status', 'DISMANTLE')
            ->where('subscription_maps', '!=', '-')
            ->whereNotNull('subscription_maps')
            ->where('subscription_maps', '!=', '');
    ...
    private function getPaginatedCustomers(Request $request)
    {
        $query = Customer::select(['customer_id', 'customer_name', 'customer_email', 'customer_phone'])
            ...
            ->whereHas('subscriptions', function ($query) {
                $query->where('subscription_status', 'DISMANTLE');
            });
    ...
    public function mapData(Request $request)
    {
        ...
        return response()->json([
            'data' => $mapData,
            'count' => count($mapData),
            'total_in_db' => $this->getTotalDismantleWithCoordinates(),
    ...
    /**
     * Get paginated table data for DISMANTLE subscriptions
     */
    ...
    /**
     * Export DISMANTLE subscriptions data
     */
    public function export(Request $request)
    {
        ...
        if ($format === 'csv') {
            $filename = 'dismantle_subscriptions_' . date('Y-m-d_H-i-s') . '.csv';
            ...
            $callback = function () use ($subscriptions) {
                ...
                fputcsv($file, [
                    ...
                    'Coordinates', 'Created At', 'DISMANTLE At'
                ]);
    ...
    /**
     * Get total count of DISMANTLE subscriptions with coordinates
     */
    private function getTotalDismantleWithCoordinates(): int
    {
        return Subscription::where('subscription_status', 'DISMANTLE')
            ->where('subscription_maps', '!=', '-')
            ->whereNotNull('subscription_maps')
            ->where('subscription_maps', '!=', '')
            ->count();
