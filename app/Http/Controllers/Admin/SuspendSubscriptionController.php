...
class SuspendSubscriptionController extends Controller
{
    /**
     * Display SUSPEND subscriptions with map view
     */
    public function index(Request $request): Response
    {
        // Only get basic statistics and initial center coordinates for map
        $stats = $this->getSuspendSubscriptionStats();
        ...
        return Inertia::render('Admin/SuspendSubscription/Index', [
            'customers' => $customers,
    ...
    private function getSuspendSubscriptionStats(): array
    {
        $totalSuspend = Subscription::where('subscription_status', 'SUSPEND')->count();

        $withCoordinates = Subscription::where('subscription_status', 'SUSPEND')
            ->where('subscription_maps', '!=', '-')
            ->whereNotNull('subscription_maps')
            ->where('subscription_maps', '!=', '')
            ->count();

        $withoutCoordinates = $totalSuspend - $withCoordinates;

        return [
            'total_suspended' => $totalSuspend,
    ...
    private function getInitialMapData(Request $request): array
    {
        $query = Subscription::select([
            ...
            'subscription_address', 'subscription_description', 'serv_id', 'created_at', 'suspend_at'
        ])
            ->with(['customer:customer_id,customer_name,customer_phone,customer_email'])
            ->where('subscription_status', 'SUSPEND')
            ->where('subscription_maps', '!=', '-')
            ->whereNotNull('subscription_maps')
            ->where('subscription_maps', '!=', '');
    ...
    private function getPaginatedCustomers(Request $request)
    {
        $query = Customer::select(['customer_id', 'customer_name', 'customer_email', 'customer_phone'])
            ...
            ->whereHas('subscriptions', function ($query) {
                $query->where('subscription_status', 'SUSPEND');
            });
        ...
        foreach ($subscriptions as $subscription) {
            ...
            if ($coords && $subscription->customer) {
                $mapData[] = [
                    ...
                    'suspend_at' => $subscription->suspend_at ? $subscription->suspend_at->format('Y-m-d H:i') : null,
                ];
            }
        }
    ...
    public function mapData(Request $request)
    {
        ...
        return response()->json([
            'data' => $mapData,
            'count' => count($mapData),
            'total_in_db' => $this->getTotalSubscriptionWithCoordinates(),
    ...
    /**
     * Get paginated table data for SUSPEND subscriptions
     */
    public function tableData(Request $request)
    {
        ...
        $query = Subscription::select([
            ...
            'serv_id', 'created_at', 'suspend_at', 'updated_at'
        ])
            ->with(['customer:customer_id,customer_name,customer_phone,customer_email'])
            ->where('subscription_status', 'SUSPEND')
    ...
    /**
     * Export SUSPEND subscriptions data
     */
    public function export(Request $request)
    {
        ...
        if ($format === 'csv') {
            $filename = 'suspend_subscriptions_' . date('Y-m-d_H-i-s') . '.csv';
            ...
            $callback = function () use ($subscriptions) {
                ...
                fputcsv($file, [
                    ...
                    'Coordinates', 'Created At', 'SUSPEND At'
                ]);
                ...
                foreach ($subscriptions as $subscription) {
                    fputcsv($file, [
                        ...
                        $subscription->suspend_at ? $subscription->suspend_at->format('Y-m-d H:i:s') : '',
                    ]);
                }
    ...
    /**
     * Get total count of SUSPEND subscriptions with coordinates
     */
    private function getTotalSubscriptionWithCoordinates(): int
    {
        return Subscription::where('subscription_status', 'SUSPEND')
            ->where('subscription_maps', '!=', '-')
            ->whereNotNull('subscription_maps')
            ->where('subscription_maps', '!=', '')
            ->count();
