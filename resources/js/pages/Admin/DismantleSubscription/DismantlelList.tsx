...
export default function DismantleList({ initialSearch = '', mapBounds, onLocationClick }: DismantleListProps) {
    ...
    const fetchTableData = useCallback(
        async (page = 1, searchTerm = '', bounds: any = null, force = false) => {
            ...
            setLoading(true);
            try {
                const response = await fetch('/admin/dismantle-subscription/table-data', {
                    method: 'POST',
                    headers: {
        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2">
                    dismantled Subscriptions Data
                    {loading && <Loader2 className="h-4 w-4 animate-spin" />}
                </CardTitle>
                <CardDescription>
                    Detailed view of dismantled subscriptions
                    ...
                    {!tableData ? (
                        ...
                        <div className="py-8 text-center">
                            <p className="text-muted-foreground">
                                {search || mapBounds
                                    ? 'No dismantled subscriptions found matching your criteria.'
                                    : 'No dismantled subscriptions found.'}
                            </p>
                        </div>
                    ) : (
                        <>
