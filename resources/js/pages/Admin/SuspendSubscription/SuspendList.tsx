...
export default function SuspendList({ initialSearch = '', mapBounds, onLocationClick }: SuspendListProps) {
    ...
    const fetchTableData = useCallback(
        async (page = 1, searchTerm = '', bounds: any = null, force = false) => {
            ...
            setLoading(true);
            try {
                const response = await fetch('/admin/suspend-subscription/table-data', {
                    method: 'POST',
                    headers: {
    ...
    const columns: TableColumn[] = [
        ...
        },
        {
            ...
            render: (data: MapDataPoint) => <Badge variant="warning">{data.subscription_status}</Badge>,
        },
        {
            header: 'Location',
            className: 'min-w-[200px]',
        ...
        //             {data.suspend_at && <p className="text-sm text-red-600">suspended: {data.suspend_at}</p>}
        //         </div>
        //     ),
        // },
        {
        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2">
                    suspended Subscriptions Data
                    {loading && <Loader2 className="h-4 w-4 animate-spin" />}
                </CardTitle>
                <CardDescription>
                    Detailed view of suspended subscriptions
                    ...
                    {!tableData ? (
                        ...
                        <div className="py-8 text-center">
                            <p className="text-muted-foreground">
                                {search || mapBounds
                                    ? 'No suspended subscriptions found matching your criteria.'
                                    : 'No suspended subscriptions found.'}
                            </p>
                        </div>
                    ) : (
                        <>
